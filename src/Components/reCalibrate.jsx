...
const ReCalibrate = ({children}) => {
    ...
    useEffect(() => {
        ...
        wrappedElement.style.width = wrappedElement.style.width ? wrappedElement.style.width : window.getComputedStyle(wrappedElement).getPropertyValue("width")
        wrappedElement.style.height = wrappedElement.style.height ? wrappedElement.style.height : window.getComputedStyle(wrappedElement).getPropertyValue("height")

        // console.log(window.getComputedStyle(wrappedElement).margin)
        parentRef.current.style.width = window.getComputedStyle(wrappedElement).getPropertyValue("width")
        parentRef.current.style.height = window.getComputedStyle(wrappedElement).getPropertyValue("height")
        parentRef.current.style.margin = window.getComputedStyle(wrappedElement).margin
        // console.log(parentRef.current.style.margin)
        childRef.current.style.bottom = changeBottom(window.getComputedStyle(wrappedElement).margin)
...
function changeBottom(margin) {
    ...
    let finalMargin = margins.reduce((sum, margin) => Number(margin.slice(0, margin.length - 2)) + sum, 5)
